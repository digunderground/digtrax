...
class DigTrax {
    // Singleton
    private static instance: DigTrax;
    ...
    // Legacy OneTagger config detection — populated after dispatching `detectLegacySettings`.
    // Used by App.vue to show a one-time first-launch import toast.
    legacyConfig: Ref<{ exists: boolean, path?: string, settings?: any }> = ref({ exists: false });
    ...
    constructor() {
        // Singleton
        if (DigTrax.instance) {
            return DigTrax.instance;
        }
        DigTrax.instance = this;
    ...
    private async incomingEvent(json: any) {
        ...
        switch (json.action) {
            ...
            case 'loadSettings':
                ...
                break;
            // Legacy OneTagger settings detect — handled by App.vue's first-launch toast.
            case 'detectLegacySettings':
                this.legacyConfig.value = {
                    exists: !!json.exists,
                    path: json.path,
    ...
    async saveQTTrack() {
        ...
        for (const change of changes) {
            this.quickTag.value.saving += 1;
            this.send('quickTagSave', change as any);
        }
...
/// Get DigTrax instance
function get1t(): DigTrax {
    return new DigTrax();
}

export { DigTrax, get1t };