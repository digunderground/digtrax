...
use digtrax_shared::{VERSION, COMMIT, PORT};
...
fn main() {
    ...
    // Server mode
    if cli.server {
        digtrax_ui::start_all(context).expect("Failed to start servers!");
        return;
    }
...
fn old_macos_warning() -> Result<(), Error> {
    ...
    if version.starts_with("10.") && !version.contains("10.15") {
        let server_version = DialogBuilder::message()
            ...
            .set_text("In order to use DigTrax on older macOS, install a more recent browser like Google Chrome and set it as default browser. Click Yes to restart DigTrax into server mode and open in browser, No to open normally.")
            .confirm()
            .show()?;
