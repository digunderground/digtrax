...
use digtrax_tag::{AudioFileFormat, CoverType, Picture, Tag};
use digtrax_tag::id3::{ID3Comment, ID3Popularimeter};
...
impl TagEditor {
    ...
    // Load tags from N files. Per-file failures are reported in `failed`
    // rather than aborting the batch — partial loads are useful for the UI.
    pub fn load_files(paths: &[PathBuf]) -> Result<LoadedFileSet, Error> {
        let mut files = Vec::with_capacity(paths.len());
        let mut failed = Vec::new();
        for path in paths {
            match TagEditor::load_file(path) {
                Ok(f) => files.push(f),
                Err(e) => failed.push((path.clone(), format!("{e:#}"))),
            }
        }
        Ok(LoadedFileSet { files, failed })
    }
