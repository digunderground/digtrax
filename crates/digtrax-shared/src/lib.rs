...
/// DigTrax WebServer port
pub const PORT: usize = 36913;
/// Current DigTrax version
...
/// Setup DigTrax logging and panic hooks
pub fn setup() {
    // Fern logger setup
    ...
    Dispatch::new()
        ...
        .level_for("digtrax_shared", level)
        .level_for("digtrax_autotag", level)
        .level_for("digtrax_platforms", level)
        .level_for("digtrax_player", level)
        .level_for("digtrax_playlist", level)
        .level_for("digtrax_renamer", level)
        .level_for("digtrax_tag", level)
        .level_for("digtrax_cli", level)
        .level_for("digtrax_tagger", level)
        .level_for("digtrax_ui", level)
        .level_for("digtrax", level)
        // Custom platforms
        .level_for("digtrax_custom_platform", level)
        // Songrec fork log
        ...
        .chain(
            Dispatch::new()
                ...
                .chain(fern::log_file(Settings::get_folder().unwrap().join("digtrax.log")).unwrap())
        )
        .apply()
        .unwrap();
...
impl Settings {
    ...
    /// Get app data folder. Migration of pre-existing config is handled by the UI layer:
    /// the WS `detectLegacySettings` action surfaces any legacy settings.json on first
    /// launch, the user explicitly imports via a toast or the Settings → Backup & Restore
    /// panel. No auto-copy here — keeps this layer pure.
    pub fn get_folder() -> Result<PathBuf, Error> {
        ...
        let root = ProjectDirs::from("com", "DigTrax", "DigTrax").ok_or(anyhow!("Error getting DigTrax data dir!"))?;
        let dir = root.preference_dir();
        if !dir.exists() {
