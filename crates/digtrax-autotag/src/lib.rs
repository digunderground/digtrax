...
use anyhow::Error;
use digtrax_renamer::{Renamer, RenamerConfig, TemplateParser};
...
use chrono::Local;
use execute::Execute;
use digtrax_tagger::{FileTaggedStatus, LyricsExt, MatchReason, MatchingUtils, SupportedTag, TrackMatch};
use regex::Regex;
...
use digtrax_tag::{AudioFileFormat, Tag, Field, TagDate, CoverType, TagImpl, EXTENSIONS};
use digtrax_shared::Settings;
use digtrax_player::AudioSources;
use digtrax_tagger::{Track, AudioFileInfo, TaggerConfig, StylesOptions, AutotaggerSource, AutotaggerSourceBuilder};
...
impl TrackImpl for Track {
    ...
    fn write_to_file(&self, path: impl AsRef<Path>, config: &TaggerConfig) -> Result<(), Error> {        
        ...
        if config.tag_enabled(SupportedTag::Key) && self.key.is_some() {
            ...
            // Convert to camelot
            if config.camelot {
                value = digtrax_tagger::to_camelot(&value).to_owned();
            }
        ...
        if config.tag_enabled(SupportedTag::Genre) && !self.genres.is_empty() {
            ...
            if config.capitalize_genres {
                genres = genres.into_iter().map(|g| digtrax_shared::capitalize(&g)).collect();
            }
