...
use digtrax_platforms::{beatport, junodownload, spotify, traxsource, discogs, itunes, musicbrainz, beatsource, bpmsupreme, bandcamp, deezer, musixmatch};
use image::{ImageFormat, ImageReader};
use digtrax_shared::Settings;
use digtrax_tagger::custom::MatchTrackResult;
use digtrax_tagger::{AutotaggerSourceBuilder, PlatformInfo, AutotaggerSource, TaggerConfig, AudioFileInfo, Track, SupportedTag, TrackMatch, ConfigCallbackResponse};
...
impl CustomPlatform {
    ...
    pub fn open(path: &PathBuf) -> Result<CustomPlatform, Error> {
        let p = unsafe {
            ...
            if **version != digtrax_tagger::custom::CUSTOM_PLATFORM_COMPATIBILITY {
                warn!("Plugin is incompatible! Plugin version: {}, Supported version: {}", **version, digtrax_tagger::custom::CUSTOM_PLATFORM_COMPATIBILITY);
                return Err(anyhow!("Plugin is incompatible!"));
            }
            // Setup logging
            let logging_cb_fn: Symbol<unsafe extern "C" fn(extern "C" fn (*mut digtrax_tagger::custom::FFIRecord))> = lib.get(b"_1t_register_logger")?;
            logging_cb_fn(digtrax_tagger::custom::write_log);
            // Get builder
