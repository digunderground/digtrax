...
#[macro_use] extern crate digtrax_shared;

use anyhow::Error;
use digtrax_ui::StartContext;
use std::fs::File;
...
use digtrax_renamer::{RenamerConfig, Renamer, TemplateParser};
use digtrax_shared::{VERSION, COMMIT};
use digtrax_autotag::audiofeatures::{AudioFeaturesConfig, AudioFeatures};
use digtrax_autotag::{Tagger, TaggerConfigExt, AudioFileInfoImpl};
use digtrax_tagger::{TaggerConfig, AudioFileInfo, SupportedTag};
...
fn main() {
    ...
    if cli.action.is_none() {
        println!("No action. Use digtrax-cli --help to get print help.");
        return;
    }

    // Setup logging
    digtrax_shared::setup();
    info!("\n\nStarting DigTrax v{VERSION} Commit: {COMMIT} OS: {}\n\n", std::env::consts::OS);
    ...
    match &action {
        Actions::Autotagger { path, .. } => {
            ...
            let files = if path.is_file() {
                digtrax_playlist::get_files_from_playlist_file(path).expect("Not a valid playlist file")
            } else {
            ...
            match prompt {
                ...
                false => {
                    ...
                    std::thread::spawn(move || {
                        digtrax_ui::start_all(StartContext {
                            server_mode: true,
                            start_path: None,
                            expose,
                            browser: false,
