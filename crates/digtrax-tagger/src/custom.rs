...
pub extern "C" fn write_log(record: *mut FFIRecord) {
    unsafe {
        ...
        let target = format!("digtrax_custom_platform::{}", (*record).target);
...
macro_rules! create_plugin {
    ($builder_type:ty, $plugin_type:ty) => {
        ...
        #[no_mangle]
        pub static _1T_PLATFORM_COMPATIBILITY: i32 = digtrax_tagger::custom::CUSTOM_PLATFORM_COMPATIBILITY;

        /// 1T logging
        struct _OnetaggerCustomPlatformLogger;

        static mut _1T_LOG_FN: Option<extern fn(*mut digtrax_tagger::custom::FFIRecord)> = None;
            ...
            fn log(&self, record: &log::Record) {
                unsafe {
                    if let Some(f) = _1T_LOG_FN {
                        let mut record: digtrax_tagger::custom::FFIRecord = record.into();
        ...
        #[no_mangle]
        pub extern "C" fn _1t_register_logger(f: extern fn (*mut digtrax_tagger::custom::FFIRecord)) {
        ...
        #[no_mangle]
        pub extern "C" fn _1t_builder_info(ptr: *mut std::ffi::c_void) -> *mut digtrax_tagger::PlatformInfo {
            let builder: Box<$builder_type> = unsafe { Box::from_raw(ptr as *mut $builder_type) };
        ...
        pub extern "C" fn _1t_builder_get_source(
            ...
            config: &digtrax_tagger::TaggerConfig
        ) -> *mut std::ffi::c_void {
        ...
        ) -> *mut digtrax_tagger::ConfigCallbackResponse {
            let mut builder: Box<$builder_type> = unsafe { Box::from_raw(ptr as *mut $builder_type) };
        ...
        pub extern "C" fn _1t_match_track(
            ptr: *mut std::ffi::c_void,
            info: &digtrax_tagger::AudioFileInfo,
            config: &digtrax_tagger::TaggerConfig
        ) -> *mut digtrax_tagger::custom::MatchTrackResult {
            let mut source: Box<dyn digtrax_tagger::AutotaggerSource> = unsafe { 
                Box::from_raw(ptr as *mut $plugin_type) 
            };
            let r = digtrax_tagger::custom::MatchTrackResult::from_result(source.match_track(info, config));
        ...
        pub extern "C" fn _1t_extend_track(
            ptr: *mut std::ffi::c_void,
            track: &mut digtrax_tagger::Track,
            config: &digtrax_tagger::TaggerConfig
