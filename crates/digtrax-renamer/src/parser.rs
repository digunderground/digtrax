use std::path::Path;
use digtrax_tag::Tag;
use digtrax_tagger::{AudioFileInfo, Field};
    ...
    fn get_value(&self, input: Option<&Data>, _info: &AudioFileInfo, config: &RenamerConfig) -> Option<Data> {
        ...
        match &self.name.to_lowercase()[..] {
            ...
            "camelot" => {
                match data {
                    Data::String(s) => {
                        Some(Data::String(digtrax_tagger::to_camelot(s).to_owned()))
                    },
                    Data::Array(a) => {
                        Some(Data::Array(a.iter().map(|v| digtrax_tagger::to_camelot(v).to_owned()).collect()))
                    }
                }
            },
            ...
            "uncamelot" => {
                match data {
                    Data::String(s) => {
                        Some(Data::String(digtrax_tagger::from_camelot(s).to_owned()))
                    },
                    Data::Array(a) => {
                        Some(Data::Array(a.iter().map(|v| digtrax_tagger::from_camelot(v).to_owned()).collect()))
                    }
                }
            }
            f => {
